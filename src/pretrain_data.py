...
class P5_Amazon_Dataset(Dataset):
    def __init__(self, all_tasks, task_list, tokenizer, args, sample_numbers, mode='train', split='toys', rating_augment=False, sample_type='random'): 
        ...
        if self.mode == 'train':
            self.review_data = load_pickle(os.path.join(file_path, '../data', split, 'review_splits.pkl'))['train']
            self.exp_data = load_pickle(os.path.join(file_path, '../data', split, 'exp_splits.pkl'))['train']
            if self.rating_augment:
                self.rating_data = load_pickle(os.path.join(file_path, '../data', split, 'rating_splits_augmented.pkl'))['train']
            else:
                self.rating_data = self.review_data
        elif self.mode == 'val':
            self.review_data = load_pickle(os.path.join(file_path, '../data', split, 'review_splits.pkl'))['val']
            self.exp_data = load_pickle(os.path.join(file_path, '../data', split, 'exp_splits.pkl'))['val']
            if self.rating_augment:
                self.rating_data = load_pickle(os.path.join(file_path, '../data', split, 'rating_splits_augmented.pkl'))['val']
            else:
                self.rating_data = self.review_data
        elif self.mode == 'test':
            self.review_data = load_pickle(os.path.join(file_path, '../data', split, 'review_splits.pkl'))['test']
            self.exp_data = load_pickle(os.path.join(file_path, '../data', split, 'exp_splits.pkl'))['test']
            if self.rating_augment:
                self.rating_data = load_pickle(os.path.join(file_path, '../data', split, 'rating_splits_augmented.pkl'))['test']
            else:
                self.rating_data = self.review_data
            self.zeroshot_exp_data = load_pickle(os.path.join(file_path, '../data', 'beauty', 'zeroshot_exp_splits.pkl')) # change to dataset to be transferred (e.g., 'beauty')
        else:
            raise NotImplementedError
            
        self.sequential_data = ReadLineFromFile(os.path.join(file_path, '../data', split, 'sequential_data.txt'))
        ...
        self.all_users = list(user_items.keys())
        ...
        if self.mode == 'test':
            self.negative_samples = ReadLineFromFile(os.path.join(file_path, '../data', split, 'negative_samples.txt'))
            
        datamaps = load_json(os.path.join(file_path, '../data', split, 'datamaps.json'))
        self.user2id = datamaps['user2id']
        self.item2id = datamaps['item2id']
        assert(len(self.item2id) == len(self.all_item))
        assert(len(self.user2id) == len(self.all_users))
        # print('n_user: ', len(self.user2id))
        # print('n_item: ', len(self.item2id))
        
        self.user_list = list(datamaps['user2id'].keys())
        self.item_list = list(datamaps['item2id'].keys())
        self.id2item = datamaps['id2item']
        
        self.user_id2name = load_pickle(os.path.join(file_path, '../data', split, 'user_id2name.pkl'))
        
        self.meta_data = []
        for meta in parse(os.path.join(file_path, '../data', split, 'meta.json.gz')):
        ...
        for item in self.item_list:
            categories = set(self.meta_data[self.meta_dict[item]]['categories'][0]) 
            category_set = category_set.union(categories)
            if len(self.meta_data[self.meta_dict[item]]['categories'][0]) == 0:
                print('item id with no categories')
        self.all_categories = category_set

        brand_set = set()
        for item in self.item_list:
            brand = self.meta_data[self.meta_dict[item]].get('brand', 'None')
            brand_set.add(brand)
        self.all_brands = list(brand_set)
        self.total_length = 0
        self.datum_info = []
            ...
            if key == 'rating':
                ...
                if sum([0 < int(ind.split('-')[1]) <= 7 for ind in self.task_list[key]]):
                    ...
                    for i in range(self.total_length - curr):
                        self.datum_info.append((i + curr, key, i // self.sample_numbers[key][0]))
                    curr = self.total_length
            elif key == 'relate_inout_embeds':
                self.total_length += len(self.all_item) * self.sample_numbers[key]
                for i in range(self.total_length - curr):
                    self.datum_info.append((i + curr, key, i // self.sample_numbers[key]))
                curr = self.total_length
            elif key == 'learn_title':
                self.total_length += len(self.all_item) * self.sample_numbers[key]
                for i in range(self.total_length - curr):
                    self.datum_info.append((i + curr, key, i // self.sample_numbers[key]))
                curr = self.total_length
            elif key == 'learn_category':
                self.total_length += len(self.all_item) * self.sample_numbers[key]
                for i in range(self.total_length - curr):
                    self.datum_info.append((i + curr, key, i // self.sample_numbers[key]))
                curr = self.total_length
            elif key == 'learn_brand':
                self.total_length += len(self.all_item) * self.sample_numbers[key]
                for i in range(self.total_length - curr):
                    self.datum_info.append((i + curr, key, i // self.sample_numbers[key]))
                curr = self.total_length
            elif key == 'learn_price':
                self.total_length += len(self.all_item) * self.sample_numbers[key]
                for i in range(self.total_length - curr):
                    self.datum_info.append((i + curr, key, i // self.sample_numbers[key]))
                curr = self.total_length
            elif key == 'learn_user':
                self.total_length += len(self.user2id) * self.sample_numbers[key]
                for i in range(self.total_length - curr):
                    self.datum_info.append((i + curr, key, i // self.sample_numbers[key]))
                curr = self.total_length
            else:
    ...
    def __getitem__(self, idx):
        ...
        if task_name == 'rating':            
            ...
            if self.mode == 'train':
                ...
                purchase_history = [f'<item_{item}>' for item in purchase_history]
            ...
            if task_template['id'] == '2-1':
                ...
                if self.mode in ['train', 'val']:
                    ...
                    while len(candidate_samples) < candidate_num:
                        ...
                        sample_ids = [f'<item_{str(item)}>' for item in sample_ids if item not in user_seq and item not in candidate_samples]
                    ...
                    candidate_samples = [f'<item_{str(item)}>' for item in candidate_samples]
                else:
                    raise NotImplementedError
                candidate_samples.extend([f'<item_{str(target_item)}>'])
            ...
            if task_template['id'] == '5-1':
                ...
                raise NotImplementedError
        elif task_name == 'relate_inout_embeds':
            item_id = self.all_item[datum_idx]
            # item_id = 1
            task_template = self.all_tasks['relate_inout_embeds']['6-1']
            assert task_template['task'] == 'relate_inout_embeds'
            source_text = task_template['source'].format(item_id)
            target_text = task_template['target'].format(item_id)
            loss_weight = 0.2
        elif task_name == 'learn_title':
            item_id = self.all_item[datum_idx]
            # item_id = 1
            task_template = self.all_tasks['learn_title']['7-1']
            assert task_template['task'] == 'learn_title'
            title = self.meta_data[self.meta_dict[self.id2item[str(item_id)]]].get('title', self.id2item[str(item_id)])
            candidate_num = random.randint(10, 20)
            sample_ids = np.random.choice(self.all_item, candidate_num, replace=False)
            sample_ids = [f'<item_{str(cand)}>' for cand in sample_ids]
            if not f'<item_{str(item_id)}>' in sample_ids:
                sample_ids = sample_ids[:-1]
                sample_ids.append(f'<item_{str(item_id)}>')
            random.shuffle(sample_ids)
            item_str = ','.join(sample_ids)
            source_text = task_template['source'].format(title, item_str)
            target_text = task_template['target'].format(item_id)
            loss_weight = 0.2
        elif task_name == 'learn_category':
            item_id = self.all_item[datum_idx]
            # item_id = 1
            task_template = self.all_tasks['learn_category']['8-1']
            assert task_template['task'] == 'learn_category'
            categories = set(self.meta_data[self.meta_dict[self.id2item[str(item_id)]]]['categories'][0]) 
            if random.random() < 0.5:
                category = random.choice(tuple(categories))
                target = 'yes'
            else:
                negs = self.all_categories.difference(categories)
                category = random.choice(tuple(negs))
                target = 'no'
            source_text = task_template['source'].format(f'<item_{item_id}>', category)
            target_text = task_template['target'].format(target)
            loss_weight = 0.2
        elif task_name == 'learn_brand':
            item_id = self.all_item[datum_idx]
            task_template = self.all_tasks['learn_brand']['9-1']
            assert task_template['task'] == 'learn_brand'
            brand = self.meta_data[self.meta_dict[self.id2item[str(item_id)]]].get('brand', 'None')
            if brand == '':
                brand = 'None'
            candidate_num = random.randint(10, 20)
            cand_brands = np.random.choice(self.all_brands, candidate_num, replace=False).tolist()
            if not brand in cand_brands:
                cand_brands = cand_brands[:-1]
                cand_brands.append(brand)
            random.shuffle(cand_brands)
            cand_brands = ', '.join(cand_brands)
            source_text = task_template['source'].format(f'<item_{item_id}>', cand_brands)
            target_text = task_template['target'].format(brand)
            loss_weight = 0.2
        elif task_name == 'learn_price':
            item_id = self.all_item[datum_idx]
            task_template = self.all_tasks['learn_price']['10-1']
            assert task_template['task'] == 'learn_price'
            price = self.meta_data[self.meta_dict[self.id2item[str(item_id)]]].get('price', 'None')
            if not price == 'None':
                price = str(int(float(price)))
            source_text = task_template['source'].format(f'<item_{item_id}>')
            target_text = task_template['target'].format(price)
            loss_weight = 0.2
        elif task_name == 'learn_user':
            user_id = self.all_users[datum_idx]
            # item_id = 1
            task_template = self.all_tasks['learn_user']['11-1']
            assert task_template['task'] == 'learn_user'
            name = self.user_id2name[user_id]
            candidate_num = random.randint(10, 20)
            sample_ids = np.random.choice(self.all_users, candidate_num, replace=False)
            sample_ids = [f'<user_{str(cand)}>' for cand in sample_ids]
            if not f'<user_{str(user_id)}>' in sample_ids:
                sample_ids = sample_ids[:-1]
                sample_ids.append(f'<user_{str(user_id)}>')
            random.shuffle(sample_ids)
            user_cand_str = ', '.join(sample_ids)
            source_text = task_template['source'].format(name, user_cand_str)
            target_text = task_template['target'].format(user_id)
            loss_weight = 0.2
...
def get_loader(args, task_list, sample_numbers, tokenizer = None, split='toys', mode='train', 
               batch_size=16, workers=4, distributed=False):
    if 't5' in args.backbone and tokenizer is None:
