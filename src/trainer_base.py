...
class TrainerBase(object):
    ...
    def create_optimizer_and_scheduler(self):
        ...
        lr_scheduler = None
        print('args optim: ', self.args.optim)
    ...
    def create_embedding_optimizer_and_scheduler(self):
        if self.verbose:
            print('Building Optimizer')
        ...
        if 'adamw' in self.args.optim:
            ...
            optimizer_grouped_parameters = [
                {
                    "params": [p for n, p in self.model.named_parameters() if 'shared' in n],
                    "weight_decay": 0,
                }
            ]
            print([n for n, p in self.model.named_parameters() if 'shared' in n])
            optim = AdamW(optimizer_grouped_parameters,
                          lr=self.args.lr, eps=self.args.adam_eps)
            # optim = SGD(optimizer_grouped_parameters,
            #               lr=self.args.lr, momentum=0.8)
            lr_scheduler = get_linear_schedule_with_warmup(
                optim, warmup_iters, t_total)
