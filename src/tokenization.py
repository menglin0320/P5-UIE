...
class P5Tokenizer(T5Tokenizer):
    def __init__(
        ...
        **kwargs
        ...
        if user_extra_ids > 0:
            additional_special_tokens.extend(["<user_{}>".format(i) for i in range(user_extra_ids)])
        
        if item_extra_ids > 0:
            additional_special_tokens.extend(["<item_{}>".format(i) for i in range(item_extra_ids)])
    ...
    def tokenize(self, source_text, **kwargs):
        # print('p5 wrapper reached')
        raw_res = super().tokenize(source_text, **kwargs)
        if not self._user_extra_ids > 0:
            return raw_res
        start_ind = -1
        ret = []
        for i in range(0, len(raw_res) - 1):
            if raw_res[i] == '<':
                if raw_res[i+1] in ['i', 'user', 'extra']:
                    start_ind = i
            if start_ind == -1:
                ret.append(raw_res[i])
            elif raw_res[i] == '>':
                ret.append(''.join(raw_res[start_ind: i+1]))
                start_ind = -1
        if start_ind == -1:
            ret.append(raw_res[-1])
        elif raw_res[-1] == '>':
            ret.append(''.join(raw_res[start_ind:]))
        else:
            # deal with corner case, should never reach here.
            ret.extend(raw_res[start_ind: ])
        return ret
    ...
    def _convert_token_to_id(self, token):
        ...
        if token.startswith("<extra_id_"):
            ...
            match = re.match(r"<user_(\d+)>", token)
            num = int(match.group(1))
            return self.vocab_size - num - 1 - self._item_extra_ids
        elif "<item_" in token:
            match = re.match(r"<item_(\d+)>", token)
            num = int(match.group(1))
    ...
    def _convert_id_to_token(self, index):
        ...
        if index < self.sp_model.get_piece_size():
            ...
            if index > self.sp_model.get_piece_size() + self._extra_ids + self._user_extra_ids - 1:
                token = "<item_{}>".format(self.vocab_size - 1 - index)
            elif index > self.sp_model.get_piece_size() + self._extra_ids - 1:
                token = "<user_{}>".format(self.vocab_size - self._item_extra_ids - 1 - index)
            else:
